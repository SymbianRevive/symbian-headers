 ...
 */
ODE_API IMPORT_C int dMassCheck(const dMass *m);

ODE_API IMPORT_C void dMassSetZero (dMass *);

ODE_API IMPORT_C void dMassSetParameters (dMass *, dReal themass,
			 dReal cgx, dReal cgy, dReal cgz,
			 dReal I11, dReal I22, dReal I33,
			 dReal I12, dReal I13, dReal I23);

ODE_API IMPORT_C void dMassSetSphere (dMass *, dReal density, dReal radius);
ODE_API IMPORT_C void dMassSetSphereTotal (dMass *, dReal total_mass, dReal radius);

ODE_API IMPORT_C void dMassSetCapsule (dMass *, dReal density, int direction,
		  	dReal radius, dReal length);
ODE_API IMPORT_C void dMassSetCapsuleTotal (dMass *, dReal total_mass, int direction,
			dReal radius, dReal length);

ODE_API IMPORT_C void dMassSetCylinder (dMass *, dReal density, int direction,
		       dReal radius, dReal length);
ODE_API IMPORT_C void dMassSetCylinderTotal (dMass *, dReal total_mass, int direction,
			    dReal radius, dReal length);

ODE_API IMPORT_C void dMassSetBox (dMass *, dReal density,
		  dReal lx, dReal ly, dReal lz);
ODE_API IMPORT_C void dMassSetBoxTotal (dMass *, dReal total_mass,
		       dReal lx, dReal ly, dReal lz);

ODE_API void dMassSetTrimesh (dMass *, dReal density, dGeomID g);

ODE_API IMPORT_C void dMassAdjust (dMass *, dReal newmass);

ODE_API IMPORT_C void dMassTranslate (dMass *, dReal x, dReal y, dReal z);

ODE_API IMPORT_C void dMassRotate (dMass *, const dMatrix3 R);

ODE_API IMPORT_C void dMassAdd (dMass *a, const dMass *b);
